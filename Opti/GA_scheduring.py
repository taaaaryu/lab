...
def create_Chromosome(length):
    ...
    for i in range(int(GENOM_LENGTH/SOFTWARE)):
        for j in range(SOFTWARE):
            sw_list.append(float(random.randint(0,1)))
        genom_list.append(sw_list)
        sw_list = []
    return Chromosome(genom_list, 0)

def calc_system(redundancy):
    system_avail = np.prod([1 - (1 - avail) ** int(red) for red, avail in zip(redundancy, SW_AVAIL)])
    return system_avail
...
def constraints(Chromosome):
    ...
    # リソースを超えないようにする
    if np.dot(genom_raw_sum, SW_RESOURCE) > RESOURCE:
        penalty += 50.0
    return penalty
