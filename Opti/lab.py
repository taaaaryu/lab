...
service_avail = [0.7, 0.8, 0.9, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8,0.7, 0.8, 0.9, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8]
server_avail = 0.99
...
for H in server_resource:
    for comb in all_combinations:
        ...
        while total_servers <= H:
            ...
            improvements = []
            for i in range(len(comb)):  # あるサービス組み合わせにおいて、１つのソフトウェアを冗長化
...
for i in range(len(server_resource)):
    ...
    for k in range(len(comb_result[i])):
        bar = ax2.bar(server_resource[i], redundancy_result[i][k], bottom=bottom, color=colors[k % len(colors)], label=f'Software {k+1}' if i == 0 else "")
...
ax2.set_ylabel('Software Redundancy')
...
plt.title(f'System Unavailability - Server Resource (number of service = {len(services)})')
